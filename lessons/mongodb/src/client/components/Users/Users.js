...
const Users = () => {
  const { onClickHandler, onDialogClose, state } = userHook();
  ...
  return (
    <>
      <div className="users">
        ...
        {users && (
          <p className="users__total">Total users {users.info.results}</p>
        )}
        {users &&
          users.results.map((r, i) => (
            <div key={r._id} className="users__user">
              <a
                className="users__user__a"
